...
const Feature = () => {
  ...
  const Shortener = (
    <div className="shortener col-12">
      <div className="container">
        <div className="shortener__block d-flex flex-wrap align-items-center">
          <div className="flex-fill col-lg-10">
            <input 
              type="text" 
          ...
          <div className="col-lg-2 my-1">
            <button className="shortener__button btn w-100" onClick={()=> shortenLink(link)}>Shorten it!</button>
          </div>
        </div>
      </div>
    </div>
  ...
  const displayLinks = links.map((l, index) => (
    <div key={index} className="shortener__links col-12 row d-flex flex-wrap align-items-center">
      <div className="shortener__links__long col-md-6">
        {l.fullLink}
      </div>
      <hr className="d-sm-none" />
      {l.isShortened ? (
        <>
        <div className="shortener__links__short col-md-4">
        {l.shortLink}
        </div>
        <div className="shortener__links__button col-md-2">
          <button 
            className={'btn w-100 ' + (l.isCopied ? 'custom-button-copied' : 'cutsom-button-copy')} 
            onClick={() => {
              navigator.clipboard.writeText(l.shortLink);
              setLinks(prevLinks => {
                let links = prevLinks;
        ...
        <div className="shortener__links__short col-md-4 ml-auto">Shortening...</div>
      )}
    </div>
  ));

  const features = feature_blocks.map((f, index) => (
    <div key={index} className="col-lg-4 my-5">
      <div className="feature__square">
  ...
  return (
    <section className="feature">
      ...
      <div className="container px-3 pb-5">
        ...
        <div className="feature__row row">
          <div className="feature__bar col-11"/>
          {features}
        </div>
      </div>
    </section>
